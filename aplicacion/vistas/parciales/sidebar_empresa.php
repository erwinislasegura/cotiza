...
<aside class="sidebar sidebar-app p-3 border-end bg-white">
  ...
  <nav class="nav flex-column small gap-1">
    <?php foreach ($items as $item): $activo = str_starts_with($rutaActual, $item['url']); ?>
      <a class="nav-link d-flex align-items-center gap-2 <?= $activo ? 'active' : '' ?>" href="<?= e(url($item['url'])) ?>">
